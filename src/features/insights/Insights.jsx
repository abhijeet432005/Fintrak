...
const useInsights = (transactions) =>
  useMemo(() => {
    ...
    const topCategoryPct = totalExpense > 0
    ...
    const topIncomeSource = Object.entries(incomeCatTotals).sort((a, b) => b[1] - a[1])[0]

    // Biggest single transaction
    const biggestIncome = incomes.reduce((max, t) => t.amount > max.amount ? t : max, incomes[0])

    // Daily average spend
    const dates    = expenses.map(t => t.date).sort()
    const firstDay = new Date(dates[0])
    const lastDay  = new Date(dates[dates.length - 1])
    const daySpan  = Math.max(1, Math.round((lastDay - firstDay) / 86400000) + 1)
    const dailyAvg = (totalExpense / daySpan).toFixed(0)

    // Expense-to-income ratio
    const ratio = totalIncome > 0 ? ((totalExpense / totalIncome) * 100).toFixed(0) : 100
    ...
    const insights = [
      {
        ...
        color: Number(savingsRate) >= 20 ? "emerald" : Number(savingsRate) >= 10 ? "amber" : "rose",
      },
      {
        key:   "topSpend",
        icon:  icons.topSpend,
      ...
      },
      {
        ...
        color: Number(ratio) > 80 ? "rose" : Number(ratio) > 60 ? "amber" : "emerald",
      },
      {
        key:   "bigIncome",
...
const InsightItem = ({ insight, index }) => {
  ...
  return (
    <div
      ...
      style={{ animation: `insightIn 0.4s ease both`, animationDelay: `${index * 60}ms` }}
    >
      <span className="text-base mt-0.5 shrink-0 select-none">{insight.icon}</span>
  ...
  <div className="rounded-2xl p-5 border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.03]">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="h-12 rounded-xl bg-zinc-100 dark:bg-white/[0.04] animate-pulse" />
      ))}
    </div>
  </div>
)
  ...
  return (
    <div className="rounded-2xl p-5 border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.03]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-semibold text-zinc-700 dark:text-white/75">Smart Insights</h3>
          <p className="text-xs text-zinc-400 dark:text-white/25 mt-0.5">Based on your transactions</p>
        </div>
