...
// ── Admin actions ─────────────────────────────────────────────────────────────
const AdminActions = ({ visible, onEdit, onDelete }) => {
  const [confirming, setConfirming] = useState(false)

  // Once confirming, always show regardless of hover/touch state
  if (confirming) {
    return (
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button
          onClick={onDelete}
          className="px-2 py-1 rounded-lg text-[11px] font-medium whitespace-nowrap
            bg-rose-500 text-white hover:bg-rose-600 transition-colors duration-150"
        >
          Yes, delete
        </button>
      </div>
  ...
  return (
    <div className="flex items-center overflow-hidden shrink-0">
      <button
        onClick={onEdit}
        title="Edit"
        className={`rounded-lg flex items-center justify-center h-7 overflow-hidden
          text-zinc-400 dark:text-white/25
          hover:bg-blue-50 dark:hover:bg-blue-500/10
          hover:text-blue-500 dark:hover:text-blue-400
          transition-all duration-200
          ${visible ? 'w-7 opacity-100 ml-1' : 'w-0 opacity-0 ml-0'}`}
      >
        <PencilIcon />
      </button>
      <button
...
const TransactionRow = ({ item, index, isAdmin = false, onDelete }) => {
  ...
  const actionsVisible = hovered || touched

  return (
    <>
      <div
        // Desktop hover via pointer events (excludes touch)
        onPointerEnter={(e) => { if (e.pointerType !== 'touch') setHovered(true)  }}
        onPointerLeave={(e) => { if (e.pointerType !== 'touch') setHovered(false) }}
        // Mobile: tap row body to toggle buttons
        onClick={(e) => {
          if (e.nativeEvent?.pointerType === 'touch') setTouched((t) => !t)
        }}
        className="group flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-3.5
        ...
        style={{ animation: "rowIn 0.3s ease both", animationDelay: `${index * 35}ms` }}
      >
        {/* Avatar */}
        <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center
          text-base sm:text-lg shrink-0 ${cfg.bg}
          transition-transform duration-200 ${actionsVisible ? 'scale-105' : ''}`}
        >
          {cfg.icon}
        </div>
        ...
        {/* Category badge */}
        <div className={`hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1
          rounded-full text-xs font-medium shrink-0 ${cfg.bg} ${cfg.text}`}
        >
        ...
        {/* Type badge */}
        <div className={`hidden md:inline-flex items-center px-2.5 py-1 rounded-full
        ...
        {isAdmin && (
          <AdminActions
            ...
            onEdit={() => { setEditing(true); setTouched(false) }}
            onDelete={() => { onDelete(item.id); setTouched(false) }}
          />
        )}
      </div>
      ...
      {editing && (
        <TransactionForm
          ...
          onSubmit={(updated) => { dispatch(editTransaction(updated)); setEditing(false) }}
        />
      )}
    </>
  )
